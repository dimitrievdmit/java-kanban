...
class FileBackedTaskManagerTest extends TaskManagerTest<FileBackedTaskManager> {
    ...
    @Test
    void shouldSaveEmptyFile() throws IOException, TaskNotFoundException, TaskTimeIntersectionException {
    ...
    int[] createMultipleTasks() throws TaskTimeIntersectionException {
        int taskId = taskManager.createTask(getDefaultTask());
        int epicId = taskManager.createEpic(getDefaultEpic());
        int subTaskId = taskManager.createSubTask(getDefaultSubTask(epicId));
        int taskIdNoTime = taskManager.createTask(getDefaultTask(NEW_START_TIME.plus(DURATION).plusDays(5)));

        return new int[]{taskId, epicId, subTaskId, taskIdNoTime};
    }

    @Test
    void shouldSaveMultipleTasks() throws IOException, TaskNotFoundException, TaskTimeIntersectionException {
    ...
    @Test
    void shouldLoadMultipleTasks() throws TaskNotFoundException, TaskTimeIntersectionException {
        int[] taskIds = createMultipleTasks();
        int taskId = taskIds[0];
        int epicId = taskIds[1];
        int subTaskId = taskIds[2];
