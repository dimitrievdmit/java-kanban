...
public class InMemoryHistoryManager implements HistoryManager {
    ...
    private List<Task> getTasks() {
        ...
        taskHistory.values().forEach(taskNode -> taskArray.add(taskNode.item));
        return taskArray;
    }
...
//    "убедитесь, что задачи, добавляемые в HistoryManager, сохраняют предыдущую версию задачи и её данных."
    private Task deepCopyTask(Task task) {
        if (task.getClass().equals(Task.class)) {
            return new Task(task.getTaskId(), task.getTitle(), task.getDescription(), task.getTaskStatus());

        } else if (task.getClass().equals(SubTask.class)) {
            return new SubTask(task.getTaskId(), task.getTitle(), task.getDescription(), task.getTaskStatus(), ((SubTask) task).getEpicId());

        } else if (task.getClass().equals(Epic.class)) {
            List<Integer> subTaskIds = ((Epic) task).getSubTaskIds();
            ArrayList<Integer> newSubTaskIds = new ArrayList<>(subTaskIds);

            return new Epic(task.getTaskId(), task.getTitle(), task.getDescription(), task.getTaskStatus(), newSubTaskIds);
        } else {
            return null;
        }
    }
